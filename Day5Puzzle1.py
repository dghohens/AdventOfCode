...
while check:
    startinglen = len(fcontent)
    print(len(fcontent))
    try:
        for i in range(len(fcontent)+1):
            if fcontent[i].lower() == fcontent[i + 1].lower():
                if fcontent[i].isupper() and fcontent[i + 1].islower():
                    fcontent = fcontent[0:i] + fcontent[i+2:]
                elif fcontent[i].islower() and fcontent[i + 1].isupper():
                    fcontent = fcontent[0:i] + fcontent[i + 2:]
    except IndexError:
        pass
    if len(fcontent) == startinglen:
        check = False
