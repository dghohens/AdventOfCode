...
whose shift most recently started. Because all asleep/awake times are during the midnight hour (00:00 - 00:59), only
...
'''
for i in fcontent:
    date = date_regex.findall(i)[0]
    date_dict.setdefault(date, [])
...
for i in range(len(fcontent)):
    if guard_regex.findall(fcontent[i]) != []:
        ...
        guard_dict.setdefault(guard_match, 0)
        for j in range(1, 9):
            try:
                ...
                if sleep_regex.findall(fcontent[i + j]) != []:
                    ...
                    guard_dict[guard_match] += (waketime - sleeptime)
            except IndexError:
                pass
...
max_guard = ''
for i in guard_dict:
    if guard_dict[i] > max_sleep:
        max_sleep = guard_dict[i]
        max_guard = i
...
for i in range(len(fcontent)):
    if guard_regex.findall(fcontent[i]) == [max_guard]:
        guard_match = guard_regex.findall(fcontent[i])[0]
        guard_sleeptime.setdefault(guard_match, [])
        for j in range(1, 9):
            try:
                if guard_regex.findall(fcontent[i + j]) != []:
                    break
                if sleep_regex.findall(fcontent[i + j]) != []:
                    ...
                    for k in range(sleeptime, waketime):
                        guard_sleeptime[guard_match].append(k)
            except IndexError:
                pass

print(guard_sleeptime)

sleepcount = counter(guard_sleeptime[max_guard])
print(sleepcount)